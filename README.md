...
- **桌宠常驻**：透明置顶 128×128 MP4 视频动画角色，四种状态自动切换——待命（slackoff 循环）、执行中（working 循环）、等待确认（slackoff）、异常（error 闪烁）；状态灯指示灯（绿/黄/红）叠加在视频右下角
- **桌宠音效**：设置 → 系统设置中可开启/关闭桌宠动画音效（默认静音），即时生效、多窗口同步
- **左键开面板**：400×600 手机式面板（项目 / 对话双 tab，默认落在对话），完整复刻主窗口的项目列表与对话能力
- **右键菜单**：打开工作台（最小化后一键找回）/ 退出应用
- **主窗口关闭行为**（设置 → 系统设置可改，支持记住选择）：
  - **每次询问**：最小化到桌宠还是完全退出，弹窗一次选定
  - **最小化到桌宠**：窗口隐藏、桌宠仍在，右键桌宠随时找回（渲染层无响应 10s 自动按最小化兜底）
  - **完全退出**：连同桌宠整体退出
- **对话双窗口实时互通**：桌宠面板 ↔ 主窗口是同一场对话的两个视口——任一端发送，另一端实时看到问题、AI 逐字打字、工具卡与子 agent 面板同步滚动；稳定点推送 + 请求收场落库校正，两端最终一致
- **thinking 全链路保真**：思考过程创建即落库（seq 时序 = 真实时序），流式显示、历史恢复、镜像同步、重启恢复四处一致
...
├── build/                          # 应用图标（ico / png）
├── public/
│   └── pet/                        # 桌宠 MP4 动画素材（idle/working/error）
├── docs/
